...
import { Plus, Trash2, Sparkles, Download, Calendar, Wallet, TrendingUp, TrendingDown, X, ClipboardList, Upload, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { RequireRole } from "@/components/require-role"
import { RekapModal } from "../../keuangan/rekap-modal"
...
const CATEGORIES = ["Pendaftaran Peserta", "Konsumsi", "Transportasi", "Perlengkapan Lomba", "Sewa Tempat", "Hadiah", "Lainnya"]
...
export default function KeuanganLkbbPage() {
  ...
  const fetchRecords = async () => {
    const { data } = await supabase.from("lkbb_financial_records").select("*").order("date", { ascending: false })
    setRecords(data || [])
  }
  ...
  const saveAll = async () => {
    ...
    setSaving(true)
    const { error } = await supabase.from("lkbb_financial_records").insert(
      valid.map((r) => ({
  ...
  const handleDelete = async (id: string) => {
    await supabase.from("lkbb_financial_records").delete().eq("id", id)
    fetchRecords()
  }
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Rekap Keuangan LKBB")
  ...
  return (
    <RequireRole path="/admin/lomba/keuangan">
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          <div>
            <Link href="/admin/lomba" className="mb-1 inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground">
              <ArrowLeft className="h-3.5 w-3.5" /> Kembali ke Peserta LKBB
            </Link>
            <h1 className="font-display text-2xl font-bold md:text-3xl">Keuangan LKBB</h1>
          </div>
      ...
      <RekapModal open={showRekap} onClose={() => setShowRekap(false)} table="lkbb_financial_records" />

      <ImportModal
        open={showImport}
        onClose={() => setShowImport(false)}
        type="keuangan"
        title="Import Data Keuangan LKBB"
        ...
        onImport={async (rows) => {
          ...
          const { error } = await supabase.from("lkbb_financial_records").insert(payload)
          setImporting(false)
